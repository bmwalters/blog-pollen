...
I perceive a productivity gain from LLM-assisted coding and I wanted to use the technique on a few of my upcoming projects. The current landscape includes a wide array of tools, but after a bit of research I settled on OpenCode as the frontend that I wanted to try. Installing OpenCode proved no easy task however. OpenCode @a[#:href "https://github.com/anomalyco/opencode/issues/3877"]{depends} on the Bun suite of JavaScript tools. Neither OpenCode nor Bun were packaged for my distro. The aur unfortunately did not come to the rescue here—the latest revision failed to build. 
...
Sure, I trust binaries built by open source GitHub actions at a level of about 99%. However I @em{completely} trust packages built by myself or signed by my distro's maintainers. I decided to pursue that incremental 1% (which I also suspected would be worthwhile @a[#:href "https://github.com/oven-sh/bun/issues/22991"]{for others} for whom the @a[#:href "https://reproducible-builds.org/"]{supply chain} is also not optimal).

@h2{Exploratory work}

I didn't have Bun on my system, but I did have Node. My first thought was to create a @code{bun-wrapper} script that would primarily log how and from where it was invoked, then also to try its best to fulfill the CLI contract expected by the build scripts using tools I did have. I reasoned that this approach would tease out the dependencies that the build scripts actually have on the system Bun install in a faster and more targeted way than grepping and reading through lots of build script code. In an ideal world, the script would evolve to have enough functionality to complete the build.
...
What follows is an edited version of @a[#:href "https://github.com/oven-sh/bun/pull/25820"]{an RFC} that I sent to the Bun maintainers detailing my approach.
...
After that came the simple matter of introducing a new CMake option to defining some @code{-DNPM_EXECUTABLE}, which defaults to @code{${BUN_EXECUTABLE}} but can be set to @code{/path/to/npm} for bootstrapping.
...
Bun's JavaScript runtime exposes several methods and classes in the @code{Bun} namespace that the JS ecosystem, including the Bun build scripts, have started to rely on.
...
Drafting the initial set of code changes was not a complex task. Most bundler options in use were easy to map. Shelling out to @code{bun build} or invoking @code{Bun.build} in code could both be replaced with @code{esbuild.build}. Where the task became more tricky was in actually assembling the binary and subsequently to load the bundled code at runtime.

@h4{zig panic}

The first post-codegen issue I encountered was a panic in the Zig compiler. One with no stack trace to boot.

My debugging strategy was to first eliminate many variables at the same time. I knew that Bun uses @a[#:href "https://github.com/oven-sh/zig"]{a fork of the Zig compiler}, and I also knew that prebuilt binaries might lack debug info and may even fail subtly on a new machine.

To move forward, I created a @a[#:href "https://github.com/bmwalters/bun/commits/dev/upstream-zig"]{side patch series} (not part of my main submission) to enable defining a local Zig compiler to use for the build, configured in a similar way to local WebKit/JavaScriptCore. This required CMake changes, but also some more interesting hacks.
...
I packaged the bootstrapped build commands as a @a[#:href "https://gist.github.com/bmwalters/090d55610d3b517bba5411335b0165fb"]{quick and dirty PKGBUILD} and successfully used the resulting build to run OpenCode.
